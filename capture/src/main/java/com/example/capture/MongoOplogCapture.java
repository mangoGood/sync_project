...
/**
 * MongoDB Oplog Capture implementation
 * Captures oplog entries from MongoDB replica set using MongoDB Java Driver
 */
public class MongoOplogCapture extends AbstractCapture<byte[]> {
    ...
    private void tailOplog() {
        try {
            // Build query to get new oplog entries
            Bson filter = lastTimestamp != null 
                ? Filters.gt("ts", lastTimestamp)
                : new Document();
            
            // Create cursor with tailable await
    ...
    /**
     * Serialize oplog entry to bytes
     */
